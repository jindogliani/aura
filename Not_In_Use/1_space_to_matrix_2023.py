...
def csv_to_heatmap(ver, spaceDataCSV, heatmap_cell_size, space_vertical_size, space_horizontal_size):
    ...
    for row in edgeArr:
        ...
        _x1, _x2, _z1, _z2= int((x1+xOffset)/heatmap_cell_size), int((x2+xOffset)/heatmap_cell_size), int((z1+zOffset)/heatmap_cell_size), int((z2+zOffset)/heatmap_cell_size) #z축 뒤집히는 것은 유동적 수정 필요
...
img = np.zeros((spaceVertcalCells, spaceHorizontalCells), dtype = np.uint8) #1px == 10cm
li = []
xOffset, zOffset = 7, 12 

for row in edgeArr:
    x1, x2, z1, z2 = -float(row[0]), -float(row[3]), float(row[2]), float(row[5])
    _x1, _x2, _z1, _z2= int((x1+xOffset)/heatmapCellSize), int((x2+xOffset)/heatmapCellSize), int((z1+zOffset)/heatmapCellSize), int((z2+zOffset)/heatmapCellSize) #z축 뒤집히는 것은 유동적 수정 필요
    img = cv2.line(img, (_x1, _z1), (_x2, _z2), 255, 1)
    wallDic = dict()
    wallDic = {'id': '', 'displayable': True, 'length':0, 'theta':0, 'x1': 0, 'z1': 0, 'x2': 0,'z2': 0}
    wallDic['length']= math.dist((x1, z1), (x2, z2))
